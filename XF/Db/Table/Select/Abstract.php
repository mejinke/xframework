...
abstract class XF_Db_Table_Select_Abstract implements XF_Db_Table_Select_Interface
{
	...
	{
		$size = isset($this->_adv_limit[0]) ? $this->_adv_limit[0] : 20;
		$offset = isset($this->_adv_limit[1]) ? $this->_adv_limit[1] : 0;

		//是否自动分页，true时URL存在分页参数时自动分页数 否则始终为第一页的数据
		$select = isset($options['select']) ? $options['select'] : FALSE;
		...
		if ($autoPage == true)
		{
			...
			//是否为正常的页码
			$page_count = ceil($dataCount/$size);
	 ...
	 * @param mixed $result
	 * @param bool $emptyObject 当没有查询到任何资料时，是否返回空对象？默认为true
	 * @return DbTable_Entity_Abstract | DbTable_Entity_Abstract Array
	 */
	protected function _packing($result , $emptyObject = true)
	{
		...
		if (!isset($result[0]))
		{
			...
			$entity->fillDataFromArray($result);
			...
			//关联完后执行指定的init
			if (method_exists($entity, 'initFromAssociateAfter') && $this->_auto_call_init)
				$entity->initFromAssociateAfter();
			...
			foreach ($fieldValues as $fk => $fv)
			{
				...
				$table = null;
			   	...
			   	foreach ($fv as $k => $v)
			   	{
			   		...
			   		$rs = $select->setWhere($v['_other']['where'])->setWhereIn($k, array_values($v['data']))->setOrder($v['_other']['order'])->setLimit(false)->fetchAll();
						...
						if ($assConfig_v['table'].'=>'.$assConfig_v['associateField'] == $assInfo_k)
						{
							...
							foreach ($assInfo_v as $assInfo_vv)
							{
								...
								foreach ($fields as $field)
								{
									if (strpos($field, ':') === 0)
										$field = substr($field, 1);
									if ($info[$info_k]->{$assConfig_k.'_'.$field} === null)
										$info[$info_k]->{$assConfig_k.'_'.$field} = false;
								}
							}
						}
					}
	...
	{	
		...
		return $this->_db_drive_connect->insert($this->_db_table->getTableName(), $this->_db_table->toArray(false));
	}
	...
	public function update(Array $data = null)
	{
		...
		if ($data == null)
			$data = $this->_db_table->toArray(false);
		$this->_db_drive_connect->showQuery($this->_show_query);
	 ...
	 * @return void
	 */
	protected function _selectDatabaseServer($dbType = 'mysql_connect')
	{
